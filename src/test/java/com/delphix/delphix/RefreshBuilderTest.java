...
public class RefreshBuilderTest {
    ...
    public void successfulBuildTest() throws Exception {
        ...
        p.getBuildersList().add(new RefreshBuilder(TestConsts.oracleEngine + "|" + TestConsts.oracleVDB2));
    ...
    public void abortedBuildTest() throws Exception {
        ...
        engine.login();
        p.getBuildersList().add(new RefreshBuilder(engine.getEngineAddress() + "|" + TestConsts.oracleVDB2));
        QueueTaskFuture<FreeStyleBuild> future = p.scheduleBuild2(0);
        future.waitForStart();
        while (!future.isDone()) {
            Thread.sleep(1000);
    ...
    public void formNoEnginesTest() throws Exception {
        FreeStyleProject p = j.createFreeStyleProject();
        p.getBuildersList().add(new RefreshBuilder("NULL"));
    ...
    /**
     * Test running a refresh job with a bad engine specified.
     */
    @Test
    ...
    public void badAddressTest() throws Exception {
        ...
        FreeStyleProject p = j.createFreeStyleProject();
        p.getBuildersList().add(new RefreshBuilder("badengine" + "|" + TestConsts.oracleVDB2));
        QueueTaskFuture<FreeStyleBuild> future = p.scheduleBuild2(0);
        while (!future.isDone()) {
            // wait for cancel to finish (needs to send cancel to Delphix Engine)
            Thread.sleep(1000);
    ...
    public void badContainerTest() throws Exception {
        ...
        FreeStyleProject p = j.createFreeStyleProject();
        p.getBuildersList().add(new RefreshBuilder(TestConsts.oracleEngine + "|" + "badcontainer"));
        QueueTaskFuture<FreeStyleBuild> future = p.scheduleBuild2(0);
        while (!future.isDone()) {
            // wait for cancel to finish (needs to send cancel to Delphix Engine)
        }
